...
pub enum Expression {
    // Regular expression variants (from ast14)
    ...
    // New do form for iteration
    Do(
        Vec<(
            (String, Range<usize>), // variable name
            Expression,             // init expression
            Option<Expression>,     // step expression (optional)
        )>, // bindings
        (Box<Expression>, Vec<Expression>), // (test, result expressions)
        Vec<Expression>,                    // body expressions
        Range<usize>,                       // span
    ),
}
