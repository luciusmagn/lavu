...
pub enum Expression {
    // Regular expression variants (from ast15)
    ...
    Do(
        Vec<((String, Range<usize>), Expression, Option<Expression>)>, // bindings: (var init step?)
        (Box<Expression>, Vec<Expression>), // test-and-result: (test result...)
        Vec<Expression>,                    // body expressions
        Range<usize>,                       // span
    ),
    // New delay form
    Delay(
        Box<Expression>, // expression to delay
        Range<usize>,    // span
    ),
}
...
impl Expression {
    pub fn span(&self) -> &Range<usize> {
        match self {
            ...
            Expression::Do(_, _, _, span) => span,
            Expression::Delay(_, span) => span,
        }
    }
}
