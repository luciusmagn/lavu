...
fn convert_definition(def: &Definition13) -> Result<Definition> {
    match def {
        Definition13::Variable((name, span), expr) => {
            let new_expr = convert_expression(expr)?;
...
fn convert_expression(expr: &Expression13) -> Result<Expression> {
    match expr {
                    ...
                    match keyword.as_str() {
                        ...
                        "let" if elements.len() >= 3 => {
                            ...
                            match &elements[1] {
                                ...
                                Expression13::Atom(Atom::Identifier(name), name_span) => {
                                    if elements.len() < 4 {
                                        bail!("Named let requires bindings and body");
                                    }

                                    // The third element should be a list of bindings
                                    if let Expression13::List(bindings_exprs, _) = &elements[2] {
        ...
        // [... all other expression variants from ast13 ...]
        Expression13::Vector(elements, span) => {
            let new_elements = elements
                .iter()
                .map(|e| convert_expression(e))
                .collect::<Result<Vec<_>>>()?;
        ...
        Expression13::Quote(inner, span) => Ok(Expression::Quote(
            Box::new(convert_expression(inner)?),
            span.clone(),
        )),

        Expression13::Quasiquote(inner, span) => Ok(Expression::Quasiquote(
            Box::new(convert_expression(inner)?),
            span.clone(),
        )),

        Expression13::Unquote(inner, span) => Ok(Expression::Unquote(
            Box::new(convert_expression(inner)?),
            span.clone(),
        )),
        ...
        Expression13::Lambda(params, body, span) => {
            let new_body = body
                .iter()
                .map(|e| convert_expression(e))
                .collect::<Result<Vec<_>>>()?;
        ...
        Expression13::SetBang(var, val, span) => {
        ...
        Expression13::If(cond, then_expr, else_expr, span) => {
        ...
        Expression13::Let(bindings, body, span) => {
        ...
        Expression13::LetStar(bindings, body, span) => {
            let new_bindings = bindings
                .iter()
                .map(|((name, name_span), expr)| {
                    let new_expr = convert_expression(expr)?;
        ...
        Expression13::LetRec(bindings, body, span) => {
            let new_bindings = bindings
                .iter()
                .map(|((name, name_span), expr)| {
                    let new_expr = convert_expression(expr)?;
        ...
        Expression13::Case(key, clauses, else_clause, span) => {
        ...
        Expression13::Cond(clauses, else_clause, span) => {
            let new_clauses = clauses
                .iter()
                .map(|(test, results)| {
        ...
        Expression13::NamedLet(name, bindings, body, span) => {
            let new_bindings = bindings
                .iter()
...
pub fn parse(
    ...
    program13: &crate::ast::ast13::Program,
) -> Result<Program> {
    let forms = program13
        .forms
        .iter()
