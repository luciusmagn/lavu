...
fn convert_definition(def: &Definition6) -> Result<Definition> {
    match def {
        Definition6::Variable((name, span), expr) => {
            let new_expr = convert_expression(expr)?;
...
fn convert_expression(expr: &Expression6) -> Result<Expression> {
    match expr {
        ...
        Expression6::Vector(elements, span) => {
            let new_elements = elements
                .iter()
                .map(|e| convert_expression(e))
                .collect::<Result<Vec<_>>>()?;
        ...
        Expression6::Quote(inner, span) => Ok(Expression::Quote(
            Box::new(convert_expression(inner)?),
            span.clone(),
        )),

        Expression6::Quasiquote(inner, span) => Ok(Expression::Quasiquote(
            Box::new(convert_expression(inner)?),
            span.clone(),
        )),

        Expression6::Unquote(inner, span) => Ok(Expression::Unquote(
            Box::new(convert_expression(inner)?),
            span.clone(),
        )),
        ...
        Expression6::Lambda(params, body, span) => {
            let new_body = body
                .iter()
                .map(|e| convert_expression(e))
                .collect::<Result<Vec<_>>>()?;
        ...
        Expression6::SetBang(var, val, span) => {
...
pub fn parse(
    ...
    program6: &crate::ast::ast6::Program,
) -> Result<Program> {
    let forms = program6
        .forms
        .iter()
