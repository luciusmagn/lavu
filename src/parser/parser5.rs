...
use crate::ast::ast4::{
    Definition as Definition4, Expression as Expression4, TopLevelForm as TopLevelForm4,
};
...
fn convert_top_level_form(form: &TopLevelForm4) -> Result<TopLevelForm> {
    match form {
        TopLevelForm4::Definition(def) => Ok(TopLevelForm::Definition(convert_definition(def)?)),
...
fn convert_expression(expr: &Expression4) -> Result<Expression> {
    match expr {
        Expression4::Atom(atom, span) => Ok(Expression::Atom(atom.clone(), span.clone())),

        Expression4::List(elements, span) => {
            // Check if this is a set! form
            if elements.len() == 3 {
                if let Expression4::Atom(Atom::Identifier(keyword), _) = &elements[0] {
                    if keyword == "set!" {
                        if let Expression4::Atom(Atom::Identifier(var_name), var_span) =
                            &elements[1]
                        {
        ...
        Expression4::UnquoteSplicing(inner, span) => Ok(Expression::UnquoteSplicing(
            Box::new(convert_expression(inner)?),
            span.clone(),
        )),
